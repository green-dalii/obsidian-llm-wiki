...
// with the deterministic one produced by `formatMentionsSection`. Honors a
// user-curated `<!-- reviewed: keep -->` marker that protects an existing
// Mentions section from being overwritten (the user has manually edited it).
...
const REVIEWED_KEEP_MARKER = '<!-- reviewed: keep -->';
 ...
 *
 * Behavior:
 *  - If body has the `<!-- reviewed: keep -->` marker, body is returned
 *    unchanged (user has curated this section manually).
...
export function injectMentionsSection(
  ...
  // User-curated override: preserve as-is.
  if (body.includes(REVIEWED_KEEP_MARKER)) return body;
