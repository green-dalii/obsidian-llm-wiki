/**
 * v1.24.1 PATCH Phase 5 — distinguishes "LLM returned 0 bytes" from "LLM gave garbage".
 *
...
/**
 * v1.24.1 PATCH Phase 5 — quiet-path options for `parseJsonResponse`.
 *
 * Both options are empty-body-only. Malformed non-empty JSON keeps the
 * legacy noisy `console.error` path (operators need that signal).
 */
export interface ParseJsonOptions {
   ...
   * are pure noise that pollutes devtools during Lint runs. Recommend
   * `true` for source-analyzer / seed-selector / lint-fix call sites
   * where empty body is expected.
   */
...
export async function parseJsonResponse(
  ...
  try {
    ...
    // v1.24.1 PATCH Phase 5: Detect empty-body SPECIFICALLY (raw bytes
    // trims to nothing after Layer-1 normalization — no `{` found means
    // no parseable payload). Empty body is "LLM returned nothing" —
    ...
    // but pure noise for empty (the response is "I called and got
    // nothing back" — it's right in the user's mental model).
    //
    ...
    // v1.24.1 PATCH Phase 5: EmptyResponseError is a domain signal we
    // deliberately throw — must propagate to caller (re-throw without
    // logging). Only UNEXPECTED exceptions get the generic catch log.
