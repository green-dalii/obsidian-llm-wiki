export const JA_TEXTS = {
    ...
    pdfBackendPreparing: 'PDF 変換を準備しています…',
    mineruRequestingUpload: 'MinerU へのアップロードを準備しています…',
    mineruUploading: 'PDF を MinerU にアップロードしています…',
    mineruWaiting: 'MinerU の処理を待っています…',
    mineruParsing: 'MinerU が PDF を解析しています…',
    mineruParsingPages: 'MinerU が {total} ページ中 {completed} ページ目を解析しています…',
    mineruConverting: 'MinerU の結果を変換しています…',
    mineruDownloading: 'MinerU の結果をダウンロードしています…',
    mineruValidating: 'MinerU の結果を確認しています…',
    mineruSaving: '変換した Markdown と画像を保存しています…',
    mineruMissingToken: '「{filename}」: 設定に MinerU API Token を追加してから、もう一度お試しください。',
    ...
    mineruDownloadFailed: '「{filename}」: MinerU の結果をダウンロードできませんでした。接続を確認して、もう一度お試しください。',
    mineruInvalidResult: '「{filename}」: MinerU から安全に使用できない結果が返されました。PDF をもう一度お試しください。',
    mineruArtifactConflict: '「{filename}」: MinerU の出力フォルダーに、このプラグインが作成していないファイルがあります。そのフォルダーを移動または名前変更して、もう一度お試しください。',
    ...
    pdfConversionBackendName: '変換バックエンド',
    pdfConversionBackendDesc: 'PDF ファイルを Markdown に変換する方法を選択します。',
    pdfConversionBackendNative: 'ネイティブモデル',
    pdfConversionBackendMineru: 'MinerU 公式 API',
    mineruApiTokenName: 'MinerU API Token',
    mineruApiTokenDesc: 'このプラグインの設定に保存され、MinerU のリクエストにのみ使用されます。',
    mineruApiTokenPlaceholder: 'MinerU API Token を貼り付け',
    mineruTaskTimeoutName: 'タスクのタイムアウト（分）',
    mineruTaskTimeoutDesc: 'MinerU タスクの待機を 5～120 分後に終了します。',
    mineruUploadDisclosure: 'MinerU 変換では PDF を MinerU の公式サービスにアップロードします。変換された Markdown と画像はその後 Vault に保存されます。',
    mineruDesktopOnlySettingDesc: 'MinerU 変換は Obsidian デスクトップ版でのみ利用できます。',
