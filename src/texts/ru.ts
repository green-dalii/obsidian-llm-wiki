...
export const RU_TEXTS = {
    ...
    taskPoliciesDesc: 'По одной записи на шаг конвейера, `шаг=режим:рассуждение`, через запятую — например `extract=text:off`. Режимы: `-` (без изменений), `schema`, `json`, `text`; рассуждение: `-`, `off`, `on`, `low`, `medium`, `high`. Пусто — встроенная основа (извлечение в текстовом режиме). Нечитаемая запись не сохраняется.',
