...
**v1.26.4 PATCH in design (6 items, user direction 2026-08-14, + #463 on 2026-08-16, + #474 + #473 on 2026-08-16):** each is a single-PR PATCH-scope fix; combine into one release. **Landed 2026-08-16: #456 (PR #462), #451 (PR #465), #463 (PR #476) MERGED.** #449 has PR #464 in review. **#474 MERGED (PR #486).** **#473 RESOLVED 2026-08-18 by removing the LLM analysis section from the LintReportModal + restoring query reasoning (PR #491).**
...
| **#473** | Final lint health-analysis sends the full `wiki/index.md` (608 KB) plus findings in one uncapped request → 313,754 tokens → LM Studio HTTP 400; Obsidian shows only `Lint failed: 400` with no report/fix dialog | **RESOLVED 2026-08-18 (PR #491). Decision (Karpathy first principles, with maintainer): remove the LLM analysis section from the LintReportModal entirely** instead of capping / token-estimating. The prompt asked the LLM to judge a whole wiki it only sees 8 pages of; schema-suggest keeps the LLM-advice path on the "Analyze Schema" button. Deletes the section + its 5 accumulated regressions (duplicate heading, leaked chain-of-thought, nested ul, repeated headings, JSON parse failure) + 1 LLM call per lint. **Side-effect fix in same PR:** #474's `prependReasoningForParse` change dropped Query reasoning — query stream paths (openai-compat 4 sites, openai-codex added) now use `wrapReasoningContent`; anthropic stream uses shared helper. Net -563 LOC; 3401 tests. Closes #473. | N/A | (1) no `## LLM 分析` heading in modal; (2) no chain-of-thought leak; (3) lint no 400 on 49K-context / 1,690-page vault; (4) query thinking panel returns |

**v1.26.4 release plan:** 6 items. **MERGED 2026-08-16: #456 (PR #462), #451 (PR #465), #463 (PR #476).** #449 = PR #464 in review (DocTpoint). **#474 MERGED (PR #486). #473 RESOLVED (PR #491).** Last open item: #449 review. Simplify + code-review on combined diff, release-skill v1.7.1 Step 5b.5 Bot pre-review BEFORE tag. Target ship date 2026-08-21 (7 days).
