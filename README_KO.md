...
**작성자:** Greener-Dalii | **버전:** 1.7.13

[English](README.md) | [中文文档](README_CN.md) | [日本語](README_JA.md) | [한국어](README_KO.md) | [Deutsch](README_DE.md) | [Français](docs/README_FR.md) | [Español](docs/README_ES.md) | [Português](docs/README_PT.md) | [공식 사이트](https://llmwiki.greenerai.top/) | [Discussions](https://github.com/green-dalii/obsidian-llm-wiki/discussions)
...
> **이전 버전에서 업그레이드하나요?** `Cmd+P` → "Regenerate index"를 실행해 alias가 포함된 Wiki 인덱스를 다시 빌드하세요 — Query에서 alias 인식 검색이 활성화됩니다(예: "DSA" 검색으로 "DeepSeek-Sparse-Attention"을 찾을 수 있음).

**수집 가속:** 많은 Entity(20개 이상)가 포함된 source의 경우 설정 → Ingestion Acceleration에서 병렬 페이지 생성을 활성화합니다:
- **Page Generation Concurrency**: 1(직렬, 가장 안전)부터 5(병렬, 가장 빠름). 대부분의 provider에서 3부터 시작합니다.
- **Batch Delay**: 병렬 배치 간 100–2000ms 지연. Rate-limited provider의 경우 500ms 이상으로 증가시킵니다.

> **안전:** 병렬 생성은 `Promise.allSettled`를 사용합니다 — 한 페이지가 실패하면 다른 페이지는 계속됩니다. 실패한 페이지는 지수 백오프와 함께 개별적으로 재시도됩니다.
