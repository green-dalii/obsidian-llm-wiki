...
| **💡 建議 Schema 更新** | `Cmd+P` → "建議 Schema 更新" — LLM 分析 Wiki 內容並建議結構改進 |
...
v1.21.0 是一個**次要功能版本**，帶來三項重大改進：預攝入需求門控阻止幻覺實體、操作歷史面板，以及 Schema 一致性 Phase 1。同時新增意大利語作爲第 9 種語言、不完整頁面清理器，以及多項 Bug 修復。

- **🛡️ 預攝入需求門控（Issue #164, PR #174）。** 每個源文件在發送給 LLM **之前**都會被驗證——空文件/純空白/僅有 frontmatter 的筆記會被拒絕，阻止小模型/本地模型（如 Ollama）幻覺實體名稱。內容哈希去重捕獲跨路徑的相同文件。交互式攝入在重新攝入重複文件前會提示確認。由 @Indexed-Apogrypha 貢獻。
- **📊 操作歷史面板（Issue #122, PR #171）。** 在可搜索、可篩選的 UI 中查看最近的攝入、Lint 報告和維護運行。洞察驅動的可視化，包含 KPI 卡片、條目詳情和可點擊的頁面鏈接。命令面板："View Ingestion History"。
- **🔗 Schema 一致性 Phase 1（Issue #124, PR #167）。** Schema 配置（`schema/config.md`）現在是系統提示詞和生成提示詞的唯一事實來源。用戶定義的自定義 section 和標籤詞表會注入到每次 LLM 調用中。
- **🇮🇹 意大利語（Issue #159, PR #159）。** 插件 UI 和 Wiki 輸出現在支持意大利語作爲第 9 種語言。由 @FrancoTampieri 貢獻。
- **🧹 不完整頁面清理器（Issue #170, PR #177）。** 因中斷攝入而留下的部分狀態 Wiki 頁面會在啓動時自動清理。頁面會歸檔到 Obsidian 的 `.trash`（可恢復）。
- **🔧 Bug 修復。** 非中文 UI 中的硬編碼中文錯誤信息（#172）；重複條目膨脹攝入報告計數（#173）。

我們強烈建議所有用戶升級到本版本，特別是使用本地 LLM（Ollama、LM Studio）的用戶——預攝入門控可阻止"空文件幻覺"這類 Bug（小模型在面對空白 prompt 時會編造實體名稱來填充 JSON schema）。

詳見 [CHANGELOG.md](../CHANGELOG.md)。

### v1.21.1 — 2026-06-22（PATCH）

- **🔧 #173 症狀 A — createOrUpdateFile 創建重試循環。** 當 `getAbstractFileByPath` 返回 null（如 macOS NFC/NFD 歸一化差異）時，3 次重試循環持續調用 `vault.create` 而非先通過 `resolveFileInVault` 解析。現在在首次嘗試時即進行路徑解析。由 @Indexed-Apogrypha 反饋。
- **📦 esbuild 0.28.0 → 0.28.1。** 修復 GHSA-g7r4-m6w7-qqqr（低嚴重性，僅開發環境）。
...
| **💡 建議 Schema 更新** | LLM 分析 Wiki 並建議 Schema 改進 |
