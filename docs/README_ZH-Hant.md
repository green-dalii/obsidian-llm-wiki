...
五個合併 PR — 錯誤修復、重構與 UX 打磨。1431 個測試通過。無新增使用者可見設定。推薦 v1.23.0+ 使用者全部升級。

- **🛠️ sources 提示候選隔離 (#234)。** LLM 候選清單預設排除 `wiki/sources/` 頁面，弱本地模型不再模糊匹配並輸出錯誤的 `[[sources/<錯誤slug>|<正確標籤>]]` 連結（導致 RAG 路由到錯誤頁面）。程式化生成的相關頁面匹配（`source-analyzer.ts:421`）不受影響。Constraints 提示詞現在顯式引用候選清單。
- **🔄 攝入時即時 PPR 圖快取失效。** 任何觸及 `wiki/` 的攝入會失效所有開啟的 Query 面板中快取的 PPR 圖 —— 同一 Obsidian 工作階段中攝入後即可查詢。
- **🔁 串流保留的客戶端封裝。** `wrapWithAdvancedSettings` 重寫為組合せ模式（`Object.create(client)` + 明確 `createMessage` 覆寫）。消除了 v1.23.0 時代的串流回歸（類別 SDK 客戶端因 `{ ...client }` 丟失原型鏈上的 `createMessageStream` 而悄悄回退為非串流）。
- **🧩 Frontmatter 序列化器整合 (DocTpoint, PR #238)。** `mergeFrontmatter` / `enforceFrontmatterConstraints` / `mergeDuplicatePages` 現在委託給統一的 `serializeFrontmatter`。行為不變（YAML 等價），但即將推出的 `supersedes:` 欄位（v1.24.0）只需在一個地方接入。
- **🖱️ Query 對話輪次指示器 + 檢索標籤 UX 打磨 (#219, #221)。** 右側每個對話輪次一個點，懸停預覽原始問題，IntersectionObserver 驅動當前可見輪次高亮，點擊捲動到該輪次。`🔍 N page(s) · …` 檢索標籤現在可點擊 —— 點擊在原位展開檢索到的頁面清單（不再彈 Notice）。生成完成後捲動到使用者問題而非答案末尾。
- **📋 語義驅動的進度通知 (#219)。** 手動操作顯示 Notice + 狀態列；背景操作（watch 模式自動攝入、週期 lint、啟動 QuickFixes）僅顯示狀態列。無新增使用者可見設定 —— 管道選擇由操作型別派生。Lint 完成通知現在遵守 `NOTICE_NORMAL` / `NOTICE_ERROR` 逾時，不再持續顯示需手動關閉。
- **📝 正規化章節標題 (DocTpoint, PR #241)。** `wikiLanguage: de` 下本地模型對章節標題的擾動（如把規範標籤 `Erwähnungen in der Quelle` 寫成 `Erwägungen…` / `Erwurnungen…`）現在通過有界 Levenshtein 距離在寫入時自動糾正回規範標籤 —— 消除從 Tier-B 檢索無聲遺失的問題。
- **📜 License 升級。** MIT → Apache License 2.0 + DCO。NOTICE 檔案列出所有人類貢獻者。既往貢獻不受追溯影響；之後的 commit 必須包含 `Signed-off-by:`。
