...
- **🔍 5 級 PPR 種子選擇級聯。** Query Wiki 現在在生成答案前執行五個互補階段（lex 快速路徑 → LLM 關鍵詞 → 本地子串掃描 → LLM KB 回退 → PPR 圖擴展）。多跳問題無需 opt-in 嵌入即可取得圖感知上下文。
- **🤫 空回應靜默路徑。** `parseJsonResponse` 在 Lint/Query 路徑遇到空 LLM 回應主體時不再記錄嘈雜錯誤，修復部分使用者回報的控制台洗頻問題（#255、#274）。種子選擇器在空主體時也會更早拋出，以便清晰恢復（#275）。
- **🧹 更乾淨的實體頁面。** 從實體頁面生成提示與 schema 中移除了冗餘的 `## 基本信息` / `## Basic Info` 區塊；新實體頁面從 frontmatter 直達 H1 → 描述 → 相關章節（#258）。
- **☁️ Bedrock Stage 1 提供商。** 新增 `bedrock-anthropic` 與 `bedrock-openai` 選項，經 AWS bedrock-mantle 端點路由。零新增 npm 依賴，套件僅約 +3 KB。
- **🦙 LM Studio 無 Key 攝入。** 攝入現在支援 LM Studio 留空 API Key，與連線測試行為一致。
- **🏗️ 內部清理。** `page-factory.ts` 拆分為 10 個聚焦模組（+99 個測試）；非破壞性 Mentions 再攝入在合併時保留較早來源的引用（#267）。

**升級提示：** 若你在 v1.24.0 手動添加了 `<!-- reviewed: keep -->` 標記，請改用 frontmatter `reviewed: true` —— 它保護整個頁面，且不會被 Markdown linter 破壞。
