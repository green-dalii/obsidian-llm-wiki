...
五個合併 PR — 錯誤修復、重構與 UX 打磨。推薦 v1.23.0+ 用戶全部升級。

- **🔄 攝入時即時 PPR 圖快取失效** — 同一會話中攝入後即可查詢
- **🔁 串流保留的用戶端封裝** — 消除了 v1.23.0 時代的串流回歸
- **🖱️ Query 對話輪次指示器 + 可點擊檢索標籤 (#221, #219)** — 右側點指示器、懸停預覽、滾動到問題。檢索標籤原位展開
- **📋 語義驅動的進度通知 (#219)** — 手動操作顯示 Notice + 狀態列；後臺操作僅顯示狀態列。Lint 通知自動關閉 (5–8s)
- **🧩 Frontmatter 序列化器整合 (PR #238 @DocTpoint)** — 統一 `serializeFrontmatter`
- **📝 規範化章節標題 (PR #241 @DocTpoint)** — 有界 Levenshtein 寫入時糾正亂碼標題
- **📜 License 升級** — MIT → Apache 2.0 + DCO。NOTICE 列出全部 6 位貢獻者

### v1.23.1 — 2026-07-02 (PATCH)

解決三項 Obsidian 審核發現：`strictBindCallApply: true` 對齊、廢棄程式碼刪除、lockfile 重新生成。無用戶可見變更。

### v1.23.0 — 2026-07-02 (MINOR)

自 1.0 以來最大的架構變更。兩大主題：

- **🤖 Vercel AI-SDK v6 遷移。** 手寫 1625-LOC LLM 用戶端替換為 `@ai-sdk/openai@3` / `@ai-sdk/anthropic@3` / `@ai-sdk/openai-compatible@2`。消除整個類別的 Provider 版本回歸 (#137 / #141 / #143 / #147 / #207)
- **🕸️ 圖引擎 — 基於 `[[wiki-link]]` 的個性化 PageRank。** Monte-Carlo PPR，零 embedding 成本，離線工作，適用於每個 Provider。三層管線 + hub 連結區分度掃描
- **🎬 所有 Provider 即時串流輸出** — 真實逐塊串流
- **📥 多選文件攝入 UI (#130)** — 雙欄選擇器 + 即時佇列 + 按文件取消
- **🎉 歡迎筆記** — 三級首次執行體驗，D8 LLM 動態翻譯（無硬編碼 i18n）
- **🔑 LM Studio API-key gate (#223)** — 本地 Provider 無需 API key 測試連線
- **🛡️ GPT-5.x Pro 路由 + URL 回退 + token-key 探針重試** — 全面 Provider 錯誤加固
...
**這個外掛到底能做什麼？**
你將筆記放入 `sources/`，LLM 提取實體和概念並生成互聯的 Wiki 頁面，帶 `[[雙向連結]]`。可以提問，從*你的*筆記中獲得對話式答案——而非網際網路搜尋。
...
**如何開始使用？**
從 Obsidian 社群外掛市場安裝 → 選擇 LLM Provider → **測試連線** → 在 `sources/` 中放入筆記 → **攝入單個來源檔案**。數秒內即可生成首批 Wiki 頁面。詳見[快速開始](#-快速開始)。

**如何控制 API 成本？**
使用粗略或極簡提取粒度進行批次攝入（更少 LLM 呼叫）。批次智慧跳過自動偵測已處理檔案。自動維護預設關閉（按需開啟）。Lint 在執行修復前顯示計數——不經你確認不會產生費用。

**我的現有 Wiki 安全嗎？**
✅ 自 v1.0.0 向後相容。在任何頁面設定 `reviewed: true` 以保護不被覆蓋。外掛從不修改你的原始檔案（`sources/`），僅修改生成的 Wiki 頁面。
