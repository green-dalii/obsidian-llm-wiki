...
四大主題：僅快取 PDF 擷取、本地模型選型指南、PDF 轉錄 Prompt 集中化、八項 e2e bug 修復。推薦 v1.24.x 使用者全部升級。

- **📄 PDF 擷取 (Level 1)。** 從 vault 選中一份 PDF —— 外掛透過你的 LLM Provider 的原生檔案輸入（anthropic / openai / bedrock-anthropic / bedrock-openai；若使用其他 OpenAI/Anthropic 相容端點，需在 Settings → LLM Configuration → Advanced 中開啟 **Force PDF Support**）讀取，按 OCR 風格的逐字轉錄轉換為 Markdown，再回到標準 Markdown 擷取流程。所有既有實體/概念/別名/`[[wiki-link]]` 工作流不變。結果以 **內容雜湊快取** 形式存放於 `.obsidian/plugins/karpathywiki/pdf-cache/`（快取鍵內嵌 `converterVersion`，prompt 升級時自動失效舊條目）。本地推薦設定見 [本地 PDF OCR 路徑](#-本地-pdf-ocr-路徑-v1250)。
- **🗄️ 有界快取增長。** 三層防禦快取治理（總計 100 MB / 1000 條 / 單條 10 MB 上限）+ LRU-by-mtime 淘汰。啟動時與每次批次擷取開始時清理舊條目。預設僅快取 —— 不修改 vault。
- **📝 可選 vault sidecar（進階）。** Settings → Wiki Configuration → Wiki Folder → **Write PDF Markdown to Vault** 在轉換後於來源 PDF 旁寫一份 `<basename>.pdf.md`。預設關閉。
- **🦙 本地模型推薦。** 模型選擇建議現分為獨立的本地與雲端章節，涵蓋 Qwen3.5 / Qwen3.6 / Gemma 4（參數與品質取捨、MLX 與 GGUF 量化、上下文策略）。
- **🛡️ 逐字 PDF 轉錄 Prompt。** PDF→Markdown Prompt 重寫為 OCR 風格的逐字轉換，搭配 `[illegible]` / `[figure: ...]` / `[equation: ...]` 反幻覺標記；把輸出包在 ```markdown 圍欄內的小模型/本地模型會在寫入快取前自動清洗。Prompt 已集中至 `src/wiki/prompts/pdf.ts`，與專案內其他 LLM 呼叫 Prompt 一致。
- **⏹ 可取消 PDF 擷取。** 轉換過程中點擊狀態列可在約 200 ms 內透過 Vercel AI SDK v6 的 AbortSignal 中斷正在進行的 LLM 呼叫。
- **🌐 i18n 完整性** — 每語系新增 10 個鍵，涵蓋兩個新設定、PDF 擷取與本地 PDF OCR 路徑（Force PDF Support 開關、Write PDF Markdown to Vault 開關、source-rejected-pdf-unsupported Notice）。

**建議檢查的設定項：** Force PDF Support（Settings → LLM Configuration → Advanced，預設關閉 —— 僅對非 NATIVE provider 有意義）、Write PDF Markdown to Vault（Settings → Wiki Configuration → Wiki Folder，預設關閉 —— 可選 sidecar 輸出）。

### v1.25.1 — 2026-07-20 (PATCH)

v1.25.0 之上的熱修復版本：八項 LLM 呼叫輸出與 lint 靜默丟失修復、三項大檔案拆分、一項建構驗證根因修復。推薦 v1.25.0 使用者全部升級。

- **🔕 Related 頁面路徑消除靜默資料遺失。** 此前，當 LLM 重寫 Related 頁面內文卻未重發 Mentions 段時，canonicalizer / related-link corrector / preserveExistingSections 管線是在 LLM 原始回覆上執行 —— 而非處理後的內文 —— 導致重新擷取可能靜默銷毀累積的 Mentions 內容。Related 路徑現已與 merge 路徑對齊：canonicalize → correct → preserveExistingSections。**強烈建議升級** —— 凡是有依賴筆記按來源累積 Mentions 再擷取的都會受惠。
- **🛡️ Schema 段不會被重寫丟棄。** LLM 與 schema 的偏差只體現在被要求寫的部分；頁面既有的 canonical 段塊即便被 LLM 重寫遺漏，也會原樣還原。統一收斂到跨 merge + related 路徑共用的單一 `preserveExistingSections` helper。
- **🔗 舊版 Mentions 頁面可自癒。** pre-#244 的分組 mentions 已在 parse 階段被識別，舊頁面可在下次擷取時重新變為結構化形式 —— LLM 不再幻覺出重複的 Mentions 區塊（程式注入是單一事實來源）。
- **🔌 LM Studio 無 API key 即可擷取。** 僅在本機執行的 LM Studio（`http://localhost:1234/v1`）現已無需佔位 key 即可擷取；非 LM Studio provider 仍要求顯式 API key（行為不變）。
- **🐢 main.js 體積更小、lint 更快。** 大檔案拆分：`wiki-engine.ts` 1799 → 更小（Phase C-PR1，4 個內部模組）、`settings.ts` 1439 → 更小（Phase C-PR2，8 個 section renderer）、`main.ts` 1304 → 300 LOC（Phase C-PR3，6 個 main-commands 模組）。`DiskCache<T>` 已抽出並具備有界增長（總計 100 MB / 1000 條 / 單條 10 MB 上限 + LRU-by-mtime 淘汰）。
- **🛠 建構驗證根因。** v1.25.0 npm registry 切換的真正根因是本機 `pnpm-lock.yaml` 與 CI `package-lock.json` 漂移；現在兩份 lockfile 都從同一份 `node_modules` 快照再生，確保本機建構與 Obsidian CI 建構產出一致。
- **⚠ 進度提示在錯誤時自動消失。** 單檔擷取（`ingestActiveFile`、`selectSourceToIngest`）現在在例外拋出時會隱藏持久的 `Ingesting: <basename>` Notice，避免失敗擷取將狀態列卡到下次成功擷取。
- **🧪 2274 個測試通過。** 較 v1.25.0 的 2182 提升，新增 `DiskCache<T>`、lint-fix-all-completion、page-batch-runner、graph-cache、index-generator、log-writer、section-header-canonicalizer 等模組。

**建議檢查的設定項：** 無 —— 本版本僅修復 + 重構；無新設定、無新命令、無新語系。
