...
本リリースは**クロスタイプ重複検出防止**に焦点を当てています。同一エンティティ/コンセプトが異なる Ingestion セッションで異なるタイプに分類された場合、以前は `entities/` と `concepts/` の両フォルダに重複ページが作成され——新コンテンツは静かに破棄されていました。この問題が修正されました。

**主な改善点：**

- **クロスフォルダ重複検出。** ページが反対側のフォルダに既存の場合（例：エンティティ"Foo"抽出時に `concepts/foo.md` が既存）、新コンテンツはその既存ページに統合され、重複作成されません。静かな情報損失がなくなりました。
- **履歴重複ページのブリッジ。** Fast path 1 の完全一致も反対側フォルダをチェックします。履歴的重複が存在する場合（アップグレード前に両フォルダに同名ページがあった）、別名が自動追加され警告がログされます。
- **Ingestion レポートで衝突表示。** バッチ Ingestion レポートにクロスタイプ衝突（別名として統合）がすべて表示されます——以前は集計されただけで表示されませんでした。
- **型安全 i18n。** `getText()` ヘルパーを追加し、unsafe type casts を排除。欠落した翻訳キーがコンパイル時に検出可能になりました。
- **173 ユニットテスト（+8）。** 新クロスタイプロジックと i18n ヘルパーの完全なテストカバレッジ。

**旧バージョンからアップグレード？** アップグレード後に **Lint Wiki** を1回実行するだけで、履歴クロスタイプ重複を自動修復。既存の設定はそのまま保持されます。
...
本リリースは**品質とインフラの更新**です。抽出信頼性、重複防止、ビルド検証にわたる複数の改善を含みます。

**主な改善点：**

- **クロスタイプ重複防止。** 実体/概念が異なる Ingestion セッションで異なるタイプに分類された場合、`resolvePagePath()` が反対フォルダ（entities ↔ concepts）をチェックし、既存ページに統合します。@dmarchevsky 氏に感謝。
- **複数ラウンド抽出の重複防止。** 非初回ラウンドに既抽出の名前と別名リストを注入。小規模モデルでも内部状態に依存せず重複を回避。
- **別名シード。** 抽出段階でオプションの `aliases` フィールドを追加。事前生成された別名はページ生成の種として機能。
- **ソース分析の誤った中断を修正 (#61)。** 初回バッチ検証を `||` から `&&` に変更。用語集タイプ（概念なし実体のみ）が正しく処理されるように。@Indexed-Apogrypha 氏に感謝。
- **ビルド検証合格。** CI が `npm install + npm run build` を使用、Obsidian 検証と一致。全依存関係を固定。
- **191 単体テスト（+18）。** クロスタイプロジック、i18n、バッチ正規化、別名重複除去を網羅。
