...
本リリースは**アーキテクチャ品質とテストインフラ**に焦点を当てています。コード品質、型安全性、テストカバレッジの全面的向上を実現。

**主な改善点：**

- **モデル互換性拡張 (Issues #64/#65)。** DeepSeek-R1、QwQ（推論モデル）、LM Studio完全対応。Thinkトークン除去で推論ブロックを処理。LM Studio互換性で未対応の `response_format: json_object` を削除。
- **テストインフラ拡張。** Mockインフラ（`createMockContext`、`createMockFile`）でObsidianランタイムなしでのコアエンジンユニットテスト実行可能。総テスト数が約200から400に倍増（+200テスト）。
- **TypeScript型安全完全達成。** `page-factory-core.test.ts` の8箇所の型エラー修正。プロジェクト史上初の `npx tsc --noEmit` 0エラー・0警告達成。
- **二重Gate検証メカニズム。** ESLintとTypeScript両方の通過（0エラー+0警告）を必須化。ESLint単独では型安全性を保証しない。
- **コアアーキテクチャリファクタリング。** 4つの純粋関数モジュールを `src/core/` に抽出：conflict-resolver（136行）、dead-link-detector（95行）、orphan-matcher（82行）、prompt-builders（104行）。
- **定数集中管理。** 30以上の散在マジックナンバーを `src/constants.ts`（192行）に集約。WIKI_SUBFOLDERS、通知時間、トークン予算などの意味定数を活性化。
- **Queryエンジン安定性。** `loadRelevantPages` のページ内容読み込みを3000トークン上限で制限、溢出防止。
- **ドキュメント向上。** TDD Standard、Development Protocol、ROADMAPアーキテクチャ品質計画、二重Gate検証ドキュメント追加。
- **コード品質。** 44ファイルで2576行追加、503行削除。副作用ゼロ、破壊的変更ゼロ。

**400テスト**（17テストファイル、v1.13.0以降+200）。
