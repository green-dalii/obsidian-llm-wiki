...
今回のリリースは**ローカルモデルの互換性**と**データ品質**に焦点を当て、コミュニティから報告された最も影響の大きい問題に対応します。

**主な改善点：**

- **LM Studio プロバイダー対応（新規）。** 独立したドロップダウンオプションとして LM Studio を追加。API Key は任意です（LM Studio はローカル動作では不要）。デフォルト URL は `http://localhost:1234/v1`。

- **コンテキストウィンドウ設定。** ローカルモデルユーザーは LLM 出力トークンを制限可能。4K〜1M のドロップダウン。ローカル/カスタムプロバイダーのみ表示。トランケーションリトライも制限。

- **YAML sources フィールド正規化（Issue #81）。** 6 種類の汚染パターンを自動正規化。Lint の LLM 依存フェーズ前に実行。

- **起動時クイック修正。** 旧「起動時ヘルスチェック」が low-level 書式問題を自動修正。10 秒の詳細通知付き。デフォルト ON。

- **エイリアス言語修正。** 中国語↔英語のハードコードされた翻訳ルールを、英語をリンカー言語とする方式に置き換え。

- **LM Studio HTTP 400 修正（Issue #75）。** `source-analyzer.ts:113` のシャドウ定数を排除。

- **設定 UI 最適化。** 新「LLM-Wiki Status」セクション。セクションタイトルを「LLM 設定」「Wiki設定」に簡略化。
