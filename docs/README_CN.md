...
- **📊 提取粒度**：五种选项控制 LLM 从源文件提取实体的深度：
  ...
  - **自定义**（1–500个）— 用户自定义实体/概念上限，适配特殊工作流。
...
五个合并 PR — 错误修复、重构与 UX 打磨。1431 个测试通过。无新增用户可见设置。推荐 v1.23.0+ 用户全部升级。

- **🛠️ sources 提示候选隔离 (#234)。** LLM 候选列表默认排除 `wiki/sources/` 页面，弱本地模型不再模糊匹配并输出错误的 `[[sources/<错误slug>|<正确标签>]]` 链接（导致 RAG 路由到错误页面）。程序化生成的相关页面匹配（`source-analyzer.ts:421`）不受影响。Constraints 提示词现在显式引用候选列表。
- **🔄 摄入时实时 PPR 图缓存失效。** 任何触及 `wiki/` 的摄入会失效所有打开的 Query 面板中缓存的 PPR 图 —— 同一 Obsidian 会话中摄入后即可查询。
- **🔁 流式保留的客户端封装。** `wrapWithAdvancedSettings` 重写为组合模式（`Object.create(client)` + 显式 `createMessage` 覆盖）。消除了 v1.23.0 时代的流式回归（类 SDK 客户端因 `{ ...client }` 丢失原型链上的 `createMessageStream` 而悄悄回退为非流式）。
- **🧩 Frontmatter 序列化器整合 (DocTpoint, PR #238)。** `mergeFrontmatter` / `enforceFrontmatterConstraints` / `mergeDuplicatePages` 现在委托给统一的 `serializeFrontmatter`。行为不变（YAML 等价），但即将推出的 `supersedes:` 字段（v1.24.0）只需在一个地方接入。
- **🖱️ Query 对话轮次指示器 + 检索标签 UX 打磨 (#219, #221)。** 右侧每个对话轮次一个点，悬停预览原始问题，IntersectionObserver 驱动当前可见轮次高亮，点击滚动到该轮次。`🔍 N page(s) · …` 检索标签现在可点击 —— 点击在原位展开检索到的页面列表（不再弹 Notice）。生成完成后滚动到用户问题而非答案末尾。
- **📋 语义驱动的进度通知 (#219)。** 手动操作显示 Notice + 状态栏；后台操作（watch 模式自动摄入、周期 lint、启动 QuickFixes）仅显示状态栏。无新增用户可见设置 —— 渠道选择由操作类型派生。Lint 完成通知现在遵守 `NOTICE_NORMAL` / `NOTICE_ERROR` 超时，不再持续显示需手动关闭。
- **📝 规范化章节标题 (DocTpoint, PR #241)。** `wikiLanguage: de` 下本地模型对章节标题的扰动（如把规范标签 `Erwähnungen in der Quelle` 写成 `Erwägungen…` / `Erwurnungen…`）现在通过有界 Levenshtein 距离在写入时自动纠正回规范标签 —— 消除从 Tier-B 检索静默丢失的问题。
- **📜 License 升级。** MIT → Apache License 2.0 + DCO。NOTICE 文件列出所有人类贡献者。既往贡献不受追溯影响；之后的 commit 必须包含 `Signed-off-by:`。
