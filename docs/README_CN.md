...
本次更新聚焦**本地模型兼容性**和**数据质量**，解决了社区报告的最关键问题。

**核心改进：**

- **新增 LM Studio 提供商。** 独立的下拉选项，API Key 可选（LM Studio 本地运行无需强制 Key）。默认地址 `http://localhost:1234/v1`。

- **上下文窗口设置。** 本地模型用户可限制 LLM 输出 Token，避免请求超出模型上下文窗口导致 HTTP 400。下拉选项 4K~1M，仅对本地/自定义提供商显示。同时限制截断重试，防止重试超限。

- **YAML sources 字段归一化（Issue #81）。** 6 种污染格式（外部路径、.md 后缀、别名管道、重复条目、内联数组、空 [[]] 链接）自动规范化。Lint 在进入 LLM 阶段前执行修复——报告者的 3800 页 vault 单次清理 572 文件/1616 条目。

- **启动时快速修复。** 原"启动健康检查"现主动修复低级格式问题（sources 归一化、Wiki 目录结构验证）。10 秒详细报告，含关闭引导。默认开启。

- **别名语言修复。** 移除硬编码中英互译规则，改为英文桥接 + "禁止为已有英文术语编造翻译"规则。Transformer 不会再被改为子虚乌有的"变换器"。

- **LM Studio 400 错误修复（Issue #75）。** `source-analyzer.ts` 中的影子常量 `MAX_TOKENS = 16000` 绕过了集中管理的 `MAX_TOKENS_BATCH`，现已全面集中化。

- **设置面板优化。** 新增"LLM-Wiki Status"区域。Section 标题精简为"LLM 配置"和"Wiki 配置"。LLM 并发度和批次延迟移入 LLM 配置区域。状态指示器整合为单行，移除冗余前缀。

**从旧版本升级？** 直接安装使用——零破坏性变更，无需重新配置。上下文窗口默认值为 0（无限制），云端用户不受影响。
---
