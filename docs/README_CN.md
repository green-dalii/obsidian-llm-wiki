...
本版本聚焦于**跨类型重复检测防护**。当同一实体/概念在不同摄入会话中被分类为不同类型时，之前会在 `entities/` 和 `concepts/` 两个文件夹中分别创建重复页面——新内容被静默丢弃。此问题现已修复。

**核心改进：**

- **跨文件夹重复检测。** 当页面已存在于相反目录（如提取实体"Foo"时 `concepts/foo.md` 已存在），新内容会合并到该现有页面而非创建重复。不再有静默信息丢失。
- **历史重复页面桥接。** Fast path 1 精确匹配现在也会检查相反目录。若存在历史重复（升级前两文件夹已有同名页面），会自动添加别名并记录警告。
- **摄入报告显示碰撞。** 批量摄入报告现在显示所有跨类型碰撞（已合并为别名）——之前此信息被聚合但从未展示。
- **类型安全 i18n。** 新增 `getText()` 辅助函数，消除代码中的 unsafe type casts。缺失的翻译键现在可在编译时检测。
- **173 单元测试（+8）。** 新增跨类型逻辑和 i18n 辅助函数的完整测试覆盖。
