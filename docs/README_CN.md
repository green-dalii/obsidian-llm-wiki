...
四大主题：仅缓存 PDF 摄取、本地模型选型指南、PDF 转录 Prompt 集中化、八项 e2e bug 修复。推荐 v1.24.x 用户全部升级。

- **📄 PDF 摄取 (Level 1)。** 从 vault 选中一份 PDF —— 插件通过你的 LLM Provider 的原生文件输入（anthropic / openai / bedrock-anthropic / bedrock-openai；若使用其他 OpenAI/Anthropic 兼容端点，需在 Settings → LLM Configuration → Advanced 中开启 **Force PDF Support**）读取，按 OCR 风格的逐字转录转换为 Markdown，再回到标准 Markdown 摄取流程。所有既有的实体/概念/别名/`[[wiki-link]]` 工作流不变。结果以 **内容哈希缓存** 形式存储在 `.obsidian/plugins/karpathywiki/pdf-cache/`（缓存键内嵌 `converterVersion`，prompt 升级时自动失效旧条目）。本地推荐配置见 [本地 PDF OCR 路径](#-本地-pdf-ocr-路径-v1250)。
- **🗄️ 有界缓存增长。** 三层防御缓存治理（总计 100 MB / 1000 条 / 单条 10 MB 上限）+ LRU-by-mtime 淘汰。启动时与每次批量摄取开始时清理旧条目。默认仅缓存 —— 不修改 vault。
- **📝 可选 vault sidecar（高级）。** Settings → Wiki Configuration → Wiki Folder → **Write PDF Markdown to Vault** 在转换后于源 PDF 旁写一份 `<basename>.pdf.md`。默认关闭。
- **🦙 本地模型推荐。** 模型选择指南现分为独立的本地与云端章节，覆盖 Qwen3.5 / Qwen3.6 / Gemma 4（参数与质量权衡、MLX 与 GGUF 量化、上下文策略）。
- **🛡️ 逐字 PDF 转录 Prompt。** PDF→Markdown Prompt 重写为 OCR 风格的逐字转换，配合 `[illegible]` / `[figure: ...]` / `[equation: ...]` 反幻觉标记；将输出包在 ```markdown 围栏内的小模型/本地模型在写入缓存前会自动清洗。Prompt 已集中至 `src/wiki/prompts/pdf.ts`，与项目内其他 LLM 调用 Prompt 一致。
- **⏹ 可取消 PDF 摄取。** 转换过程中点击状态栏可在约 200 ms 内通过 Vercel AI SDK v6 的 AbortSignal 中断正在进行的 LLM 调用。
- **🌐 i18n 完整性** — 每语种新增 10 个键，覆盖两个新设置、PDF 摄取与本地 PDF OCR 路径（Force PDF Support 开关、Write PDF Markdown to Vault 开关、source-rejected-pdf-unsupported Notice）。

**建议检查的设置项：** Force PDF Support（Settings → LLM Configuration → Advanced，默认关闭 —— 仅对非 NATIVE provider 有意义）、Write PDF Markdown to Vault（Settings → Wiki Configuration → Wiki Folder，默认关闭 —— 可选的 sidecar 输出）。

### v1.25.1 — 2026-07-20 (PATCH)

v1.25.0 之上的热修复版本：八项 LLM 调用输出与 lint 静默丢失修复、三项大文件拆分、一项构建验证根因修复。推荐 v1.25.0 用户全部升级。

- **🔕 Related 页面路径消除静默数据丢失。** 此前，当 LLM 重写 Related 页面正文却未重发 Mentions 段时，canonicalizer / related-link corrector / preserveExistingSections 管线是在 LLM 原始回复上运行 —— 而非处理后的正文 —— 导致重新摄入可能静默销毁累积的 Mentions 内容。Related 路径现已与 merge 路径对齐：canonicalize → correct → preserveExistingSections。**强烈建议升级** —— 凡是依赖笔记按来源累积 Mentions 再摄入的都会受益。
- **🛡️ Schema 段不会被重写丢弃。** LLM 与 schema 的偏差只体现在被要求写的部分；页面已有的 canonical 段块即便被 LLM 重写遗漏，也会原样还原。统一收敛到跨 merge + related 路径共享的单个 `preserveExistingSections` helper。
- **🔗 旧版 Mentions 页面可自愈。** pre-#244 的分组 mentions 已在 parse 阶段被识别，旧页面可在下次摄入时重新变为结构化形式 —— LLM 不再幻觉出重复的 Mentions 块（程序注入是单一事实源）。
- **🔌 LM Studio 无 API key 即可摄取。** 仅本地运行的 LM Studio（`http://localhost:1234/v1`）现已无需占位 key 即可摄取；非 LM Studio provider 仍要求显式 API key（行为不变）。
- **🐢 main.js 体积更小、lint 更快。** 大文件拆分：`wiki-engine.ts` 1799 → 更小（Phase C-PR1，4 个内部模块）、`settings.ts` 1439 → 更小（Phase C-PR2，8 个 section renderer）、`main.ts` 1304 → 300 LOC（Phase C-PR3，6 个 main-commands 模块）。`DiskCache<T>` 已抽取并具备有界增长（总计 100 MB / 1000 条 / 单条 10 MB 上限 + LRU-by-mtime 淘汰）。
- **🛠 构建验证根因。** v1.25.0 npm registry 切换的真正根因是本地 `pnpm-lock.yaml` 与 CI `package-lock.json` 漂移；现在两份 lockfile 都从同一份 `node_modules` 快照再生，确保本地构建与 Obsidian CI 构建产物一致。
- **⚠ 进度提示在错误时自动消失。** 单文件摄取（`ingestActiveFile`、`selectSourceToIngest`）现在在异常抛出时会隐藏持久的 `Ingesting: <basename>` Notice，避免失败摄取将状态条卡到下次成功摄取。
- **🧪 2274 个测试通过。** 较 v1.25.0 的 2182 提升，新增 `DiskCache<T>`、lint-fix-all-completion、page-batch-runner、graph-cache、index-generator、log-writer、section-header-canonicalizer 等模块。

**建议检查的设置项：** 无 —— 本版本仅修复 + 重构；无新设置、无新命令、无新语种。
