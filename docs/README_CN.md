...
[English](../README.md) | **简体中文** | [繁體中文](README.zh-Hant.md) | [日本語](README_JA.md) | [한국어](README_KO.md) | [Deutsch](README_DE.md) | [Français](README_FR.md) | [Español](README_ES.md) | [Português](README_PT.md) | [Italiano](README_IT.md)
...
- **🔧 #173 症状 A — createOrUpdateFile 创建重试循环。** 当 `getAbstractFileByPath` 返回 null（如 macOS NFC/NFD 归一化差异）时，3 次重试循环持续调用 `vault.create` 而非先通过 `resolveFileInVault` 解析。现在在首次尝试时即进行路径解析。由 @Indexed-Apogrypha 反馈。
- **📦 esbuild 0.28.0 → 0.28.1。** 修复 GHSA-g7r4-m6w7-qqqr（低严重性，仅开发环境）。
