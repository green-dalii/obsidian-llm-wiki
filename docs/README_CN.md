...
- [🧠 Karpathy LLM Wiki — Obsidian 插件](#-karpathy-llm-wiki--obsidian-插件)
  ...
  - [⚡ v1.23.0 更新内容](#-v1230-更新内容)
    - [v1.23.2 — 2026-07-05（最新，PATCH）](#v1232--2026-07-05最新patch)
...
2. 从下拉菜单选择 Provider（Anthropic、Anthropic 兼容、Google Gemini、OpenAI、DeepSeek、Kimi、GLM、MiniMax、Amazon Bedrock（bearer API key 或 AWS Profile / SSO）、LM Studio、Ollama、OpenRouter 或自定义）
...
五个合并 PR — 错误修复、重构与 UX 打磨。推荐 v1.23.0+ 用户全部升级。

- **🔄 摄入时实时 PPR 图缓存失效** — 同一会话中摄入后即可查询
- **🔁 流式保留的客户端封装** — 消除了 v1.23.0 时代的流式回归
- **🖱️ Query 对话轮次指示器 + 可点击检索标签 (#221, #219)** — 右侧点指示器、悬停预览、滚动到问题。检索标签原位展开
- **📋 语义驱动的进度通知 (#219)** — 手动操作显示 Notice + 状态栏；后台操作仅显示状态栏。Lint 通知自动关闭 (5–8s)
- **🧩 Frontmatter 序列化器整合 (PR #238 @DocTpoint)** — 统一 `serializeFrontmatter`
- **📝 规范化章节标题 (PR #241 @DocTpoint)** — 有界 Levenshtein 写入时纠正乱码标题
- **📜 License 升级** — MIT → Apache 2.0 + DCO。NOTICE 列出全部 6 位贡献者

### v1.23.1 — 2026-07-02 (PATCH)

解决三项 Obsidian 审核发现：`strictBindCallApply: true` 对齐、废弃代码删除、lockfile 重新生成。无用户可见变更。

### v1.23.0 — 2026-07-02 (MINOR)

自 1.0 以来最大的架构变更。两大主题：

- **🤖 Vercel AI-SDK v6 迁移。** 手写 1625-LOC LLM 客户端替换为 `@ai-sdk/openai@3` / `@ai-sdk/anthropic@3` / `@ai-sdk/openai-compatible@2`。消除整个类别的 Provider 版本回归 (#137 / #141 / #143 / #147 / #207)
- **🕸️ 图引擎 — 基于 `[[wiki-link]]` 的个性化 PageRank。** Monte-Carlo PPR，零 embedding 成本，离线工作，适用于每个 Provider。三层流水线 + hub 链接区分度扫描
- **🎬 所有 Provider 实时流式输出** — 真实逐块流式
- **📥 多选文件摄入 UI (#130)** — 双栏选择器 + 实时队列 + 按文件取消
- **🎉 欢迎笔记** — 三级首次运行体验，D8 LLM 动态翻译（无硬编码 i18n）
- **🔑 LM Studio API-key gate (#223)** — 本地 Provider 无需 API key 测试连接
- **🛡️ GPT-5.x Pro 路由 + URL 回退 + token-key 探针重试** — 全面 Provider 错误加固
...
- **🔌 多 Provider 支持** — Anthropic、Anthropic 兼容（Coding Plan）、Gemini、OpenAI、DeepSeek、Kimi、GLM、MiniMax、Amazon Bedrock（bearer API key 或 AWS Profile / SSO）、LM Studio、OpenRouter、Ollama、自定义接口
