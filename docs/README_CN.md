...
本次为 **Wiki 初始化 UX 与架构优化** 版本，聚焦首次使用体验更顺畅，以及测试基础设施持续扩展。

**核心改进：**

- **Wiki 自动初始化（Issue #80）。** 首次 LLM 连接测试成功后，插件自动创建 Wiki 文件夹结构（entities、concepts、sources、schema）。设置面板上的状态指示器（✅/⚠️）实时显示 Wiki 健康状态。新 Vault 不再出现"重新生成默认 Schema"按钮无响应的问题。

- **SSE 解析器提取。** 流式响应解析逻辑（Anthropic + OpenAI 格式）提取为 `src/core/sse-parser.ts` 共享纯函数。11 个测试覆盖两种格式、CRLF 标准化、异常 JSON 容错、`[DONE]` 终止符。

- **截断重试提取。** Token 截断重试策略（检测 `stop_reason=max_tokens` 或 `finish_reason=length`，双倍 max_tokens，重试一次）统一为 `src/core/truncation-retry.ts`。消除 LLM 客户端中 3 处重复代码。7 个测试覆盖上限行为、错误传播和警告日志。

- **测试基础设施增长。** +37 测试（共 446 个，21 个测试文件）。AnthropicClient 截断重试测试（9 个，包含 prefill 花括号恢复、MAX_TOKENS_BATCH 上限、cacheBreakpoint 透传）。Wiki 初始化测试（10 个，纯 mock，无需 Obsidian 运行时）。

- **开发质量闭环。** TDD + 规划循环正式写入 CLAUDE.md，附真实违规案例（2026-06-02）。所有新代码变更遵循 9 步闭环：深度思考 → 规划 → 写测试 → 确认 RED → 实现 → 确认 GREEN → 重构 → 4-Gate 验证 → 三无审查。

**从旧版本升级？** 直接安装使用——零破坏性变更。已有 Wiki 页面、设置和工作流全部保留。无需重新配置。
