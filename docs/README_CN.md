...
本次为**质量与基础设施更新**，涵盖提取可靠性、重复预防和构建验证的多项改进。

**核心改进：**

- **跨类型重复防护。** 实体/概念在不同摄入中被分类为不同类型时，`resolvePagePath()` 能跨文件夹（entities ↔ concepts）检查并合并内容而非创建重复。感谢 @dmarchevsky 贡献。
- **多轮提取去重。** 后续提取轮次现在会收到已提取名称和别名列表——小模型也不再依赖内部状态来记忆已提取内容。
- **别名种子。** 提取阶段支持为实体/概念可选生成别名（`aliases` 字段），预生成别名作为页面生成的种子，质量更佳。
- **源分析不再错误中止 (#61)。** 首次批次的验证从 `||` 改为 `&&`——词汇表类来源（只有实体没有概念，或反之）现在可正常处理。感谢 @Indexed-Apogrypha (Matthew Harper) 贡献。
- **构建验证通过。** CI 使用 `npm install + npm run build` 与 Obsidian 验证系统完全对齐。所有依赖已固定精确版本。
- **191 单元测试（+18）。** 跨类型逻辑、i18n 助手、批次归一化、别名去重的新增覆盖。

**从旧版本升级？** 升级后运行一次 **Lint Wiki** 即可自动修复历史跨类型重复页面。已有配置自动保留，无需重新设置。
