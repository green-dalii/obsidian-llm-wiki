...
本次为**架构质量与测试基础设施更新**，解决第三方模型审核提出的重大测试缺失问题。

**核心改进：**

- **模型兼容性扩展。** DeepSeek-R1、QwQ（推理模型）和 LM Studio 现完全支持。思考块剥离自动清理推理输出，LM Studio 兼容性修复移除不支持的参数——用户可无障碍使用这些流行模型。

- **测试基础设施扩展。** Mock 基础设施（`createMockContext`）使核心引擎模块可在无 Obsidian 运行时环境下单元测试。**总测试数从~200翻倍至400**（+200测试），覆盖之前无法测试的提取逻辑、冲突解决、页面生成。

- **TypeScript 类型安全完整。** 修复所有类型错误。双重Gate验证现要求 ESLint 和 TypeScript 同时通过——单一工具通过不足以保证生产质量。

- **核心架构重构。** 提取4个纯函数模块至 `src/core/` 目录：冲突解决器、断链检测器、孤立匹配器、提示构建器。常量集中化（192行）激活语义常量：WIKI_SUBFOLDERS、通知时长、token 预算。

- **查询引擎稳定性。** 页面内容加载上限3000 tokens防止溢出，提升大型 Wiki 的查询可靠性。

- **文档升级。** TDD 标准（"先写失败的测试"）、开发协议（"先规划，再执行"）、ROADMAP架构质量计划。双重Gate验证写入所有标准文档。

**从旧版本升级？** 直接安装使用即可——所有改动为内部重构，零破坏性变更。已有 Wiki 页面、设置、工作流全部保留。无需重新配置。
