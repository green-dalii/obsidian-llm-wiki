...
**1. 重建索引**
`Cmd+P` → **"Regenerate index"** — 重新构建 `wiki/index.md`，为每个页面条目附加别名信息，启用别名感知搜索（如搜索"DSA"能找到"DeepSeek-Sparse-Attention"）。旧版索引格式只列出页面标题。

**2. 运行 Lint Wiki**
`Cmd+P` → **"Lint Wiki"** — 扫描整个 Wiki，显示：
- **缺失别名**：没有别名的页面（v1.7.11 之前生成的所有页面）。点击 **"Complete Aliases"** 让 LLM 批量生成翻译、缩写、变体名。这对后续重复检测至关重要。
...
- **批次延迟**：从 300ms 开始。如遇限流请增大至 500–800ms。

**5. 检查新增设置项（v1.4.0–v1.7.x 引入）：**
...
> **安全说明**：并行生成使用 `Promise.allSettled` —— 某页失败不影响其他页面继续。失败页面会自动重试并指数退避。智能批量跳过（v1.7.7）自动检测已摄入文件，节省时间和 API 成本。
...
- **批次延迟**：值越低越快，但可能触发限流。从 300ms 开始；如遇 HTTP 429 错误，增大到 500–800ms。
...
### 界面语言和 Wiki 输出语言有什么区别？

- **界面语言**（设置顶部）：控制插件自身的 UI——设置标签、按钮文字、通知提示。目前支持英文和中文。
- **Wiki 输出语言**（v1.6.5 引入）：控制 LLM 撰写 Wiki 页面时使用的语言。支持 8 种语言（英/中/日/韩/德/法/西/葡）加自定义输入。你可以英文界面 + 中文 Wiki，反之亦然。

### 为什么 Query 找不到我明知存在的页面？

三个常见原因：
1. **索引过期**：执行 `Cmd+P` → **"Regenerate index"** 重新构建，包含当前页面和别名。
2. **缺少别名**：没有别名（v1.7.11 之前生成的页面），LLM 只能按精确标题匹配。运行 Lint → Complete Aliases 修复。
3. **搜索词不匹配**：尝试页面标题、别名或相关术语。LLM 做的是语义匹配而非关键词搜索——换个说法可能就找到了。

### "Smart Fix All" 做什么？按什么顺序执行？

一键智能修复按因果关系顺序执行，尽量不产生新问题：
1. **Phase 0 — 补全别名**：填充缺失别名，确保后续重复检测准确
...
- **批次延迟** 超过 500ms 会给 API 更多喘息空间，但不增加 token 消耗——只间隔调用。
