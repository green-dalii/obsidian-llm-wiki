...
- [⚡ v1.14.0 업데이트 하이라이트](#-v1140-업데이트-하이라이트)
- [✨ 주요 기능](#-주요-기능)
  - [📊 지식 품질](#-지식-품질)
  - [🛠️ 유지 관리](#️-유지-관리)
  - [💬 조회 및 피드백](#-조회-및-피드백)
...
이번 릴리스는 **아키텍처 품질과 테스트 인프라 업데이트**입니다. 제3자 모델 리뷰에서 제기된 중요한 테스트 부재 문제를 해결합니다.

**주요 개선사항：**

- **모델 호환성 확장 (Issues #64/#65)。** DeepSeek-R1、QwQ（추론 모델）、LM Studio 완전 지원. 추론 블록 자동 제거, LM Studio 호환성 복원——사용자가 이 인기 모델을 문제없이 사용 가능.

- **테스트 인프라 확장。** Mock 인프라（`createMockContext`）로 Obsidian 런타임 없이 코어 엔진 모듈 단위 테스트 가능. **총 테스트 수 ~200에서 400으로 두 배 증가**（+200 테스트），이전에 테스트 불가능한 추출 로직、충돌 해결、페이지 생성 커버.

- **TypeScript 타입 안전 완성。** 모든 타입 에러 수정. 이중 Gate 검증 ESLint와 TypeScript 모두 통과 필요——단일 도구 통과는 생산 품질에 부족.

- **코어 아키텍처 리팩터링。** `src/core/` 디렉토리에 4개 pure 함수 모듈 추출: 충돌 해결기（136행）、단일 링크 감지기（95행）、고립 페이지 매처（82행）、프롬프트 빌더（104행）. 상수 중앙화（192행）시맨틱 상수 활성화: WIKI_SUBFOLDERS, 알림 지속, token 예산.

- **쿼리 엔진 안정성。** 페이지 콘텐츠 로딩 3000 tokens 캡으로 overflow 방지, 대형 Wiki 쿼리 안정성 향상.

- **문서 업그레이드。** TDD 표준（"먼저 실패하는 테스트 작성"）、개발 프로토콜（"먼저 계획, 후 실행"）、ROADMAP 아키텍처 품질 계획. 이중 Gate 검증 모든 표준에 문서화.

**이전 버전에서 업그레이드？** 직접 설치 사용——모든 변경은 내부 리팩토링, 제로 파괴적 변경. 기존 Wiki 페이지、설정、워크플로 모두 보존. 재구성 불필요.
