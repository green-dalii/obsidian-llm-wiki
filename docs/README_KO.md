...
[English](../README.md) | [简体中文](README_CN.md) | [繁體中文](README.zh-Hant.md) | [日本語](README_JA.md) | **한국어** | [Deutsch](README_DE.md) | [Français](README_FR.md) | [Español](README_ES.md) | [Português](README_PT.md) | [Italiano](README_IT.md)
...
- **🔧 #173 증상 A — createOrUpdateFile 생성 재시도 루프。** `getAbstractFileByPath`가 null을 반환할 때(macOS NFC/NFD 정규화 불일치 등), 3회 재시도 루프가 `resolveFileInVault`로 먼저 해결하지 않고 `vault.create`를 계속 호출했습니다. 이제 첫 번째 시도에서 해결됩니다. @Indexed-Apogrypha 보고。
- **📦 esbuild 0.28.0 → 0.28.1。** GHSA-g7r4-m6w7-qqqr 패치(낮은 심각도, 개발 환경 전용)。
