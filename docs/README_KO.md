...
**네이티브 PDF 입력.** Markdown, 일반 텍스트, `.pdf`가 모든 수집 선택기에 표시됩니다. PDF는 설정된 제공업체의 네이티브 문서 입력으로 읽으며, 읽을 수 있는 텍스트가 없는 스캔 PDF나 PDF 입력을 거부하는 모델은 알림과 함께 건너뜁니다.
...
v1.24.0 모든 사용자에게 업그레이드를 권장합니다.

- **🔍 5단계 PPR 시드 선택 캐스케이드.** Query Wiki는 이제 답변 생성 전 5개의 보완 단계(Lex 빠른 경로 → LLM 키워드 → 로컬 부분 문자열 스캔 → LLM KB 폴백 → PPR 그래프 확장)를 실행합니다. 멀티홉 질문도 임베딩 옵트인 없이 그래프 인식 맥락을 얻습니다.
- **🤫 빈 응답 quiet path.** `parseJsonResponse`가 Lint/Query 경로에서 빈 LLM 본문에 대해 시끄러운 오류를 더 이상 기록하지 않아 일부 사용자가 보고한 콘솔 스팸을 해결합니다(#255, #274). 시드 선택기도 빈 본문에서 더 일찍 예외를 던져 복구를 명확히 합니다(#275).
- **🧹 더 깔끔한 엔티티 페이지.** 엔티티 페이지 생성 프롬프트와 스키마에서 중복된 `## Basic Information` / `## Basic Info` 블록을 제거했습니다. 새 엔티티 페이지는 frontmatter에서 H1 → 설명 → 관련 섹션으로 직결됩니다(#258).
- **☁️ Bedrock Stage 1 공급자.** AWS bedrock-mantle 엔드포인트를 경유하는 `bedrock-anthropic` 및 `bedrock-openai` 옵션 추가. 새 npm 의존성 없음, 번들 약 +3 KB.
- **🦙 LM Studio API Key 없이 수집.** 연결 테스트와 동일하게 LM Studio에서 API Key를 비워두고도 수집이 가능합니다.
- **🏗️ 내부 정리.** `page-factory.ts`를 10개의 집중 모듈로 분할(+99 테스트). 비파괴 Mentions 재수집은 병합 시 이전 소스 인용을 보존합니다(#267).

**업그레이드 참고:** v1.24.0에서 `<!-- reviewed: keep -->` 마커를 수동으로 추가했다면 frontmatter의 `reviewed: true`로 전환하세요 — 페이지 전체를 보호하고 Markdown linter에서도 깨지지 않습니다.
