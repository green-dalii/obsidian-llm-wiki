...
- [⚡ v1.13.0 업데이트 하이라이트](#-v1130-업데이트-하이라이트)
- [✨ 주요 기능](#-주요-기능)
  - [📊 지식 품질](#-지식-품질)
  - [🛠️ 유지 관리](#️-유지-관리)
  - [💬 조회 및 피드백](#-조회-및-피드백)
...
이번 릴리스는**크로스 타입 중복 감지 방지**에 초점을 맞추고 있습니다. 동일한 엔티티/컨셉이 다른 Ingestion 세션에서 다른 타입으로 분류된 경우, 이전에는 `entities/`와 `concepts/` 두 폴더에 중복 페이지가 생성되고——새 콘텐츠는 조용히 삭제되었습니다. 이 문제가 수정되었습니다.

**주요 개선사항:**

- **크로스 폴더 중복 감지.** 페이지가 반대 폴더에 이미 존재할 때(예: 엔티티"Foo" 추출 시 `concepts/foo.md`가 이미 존재), 새 콘텐츠는 해당 기존 페이지에 통합되며 중복 생성되지 않습니다. 조용한 정보 손실이 없습니다.
- **역사적 중복 페이지 브리지.** Fast path 1 정확 매칭도 반대 폴더를 검사합니다. 역사적 중복이 존재할 때(업그레이드 전 두 폴더에 동일 이름 페이지가 있었던 경우), alias가 자동 추가되고 경고가 로그됩니다.
- **Ingestion 리포트에 충돌 표시.** 배치 Ingestion 리포트에 모든 크로스 타입 충돌(alias로 통합됨)이 표시됩니다——이전에는 집계만 되고 표시되지 않았습니다.
- **형식 안전 i18n.** `getText()` 헬per를 추가해 unsafe type casts를 제거. 누락된 번역 키가 컴파일 시간에 감지 가능합니다.
- **173 단위 테스트(+8).** 새 크로스 타입 로직과 i18n helper의 완전한 테스트 커버리지.

**이전 버전에서 업그레이드?** 업그레이드 후 **Lint Wiki**를 1회 실행하면 역사적 크로스 타입 중복을 자동 수정합니다. 기존 설정은 그대로 유지됩니다.
...
이번 릴리스는**품질 및 인프라 업데이트**입니다. 추출 신뢰성, 중복 방지, 빌드 검증에 걸친 여러 개선 사항을 포함합니다.

**주요 개선사항：**

- **크로스 타입 중복 방지.** 동일 엔티티/컨셉이 다른 Ingestion 세션에서 다르게 분류된 경우 `resolvePagePath()`가 반대 폴더(entities ↔ concepts)를 확인하고 기존 페이지에 통합합니다. @dmarchevsky 님께 감사.
- **다중 라운드 추출 중복 방지.** 비첫 번째 라운드에 이미 추출된 이름과 alias 목록을 주입——소규모 모델도 내부 상태에 의존하지 않고 중복 방지.
- **Alias 시드.** 추출 단계에서 선택적 `aliases` 필드를 지원. 사전 생성된 alias는 페이지 생성 프롬프트의 시드로 사용.
- **소스 분석 오류 중단 수정 (#61).** 첫 번째 배치 검증을 `||`에서 `&&`로 변경——용어집 스타일(개념 없는 엔티티만) 소스가 올바르게 처리됨. @Indexed-Apogrypha 님께 감사.
- **빌드 검증 통과.** CI가 `npm install + npm run build`를 사용해 Obsidian 검증과 정확히 일치. 모든 의존성 고정.
- **191 유닛 테스트(+18).** 크로스 타입 로직, i18n, 배치 정규화, alias 중복 제거 커버리지 추가.
