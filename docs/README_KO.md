...
- [⚡ v1.16.0 업데이트 하이라이트](#-v1160-업데이트-하이라이트)
- [✨ 주요 기능](#-주요-기능)
  - [📊 지식 품질](#-지식-품질)
  - [🛠️ 유지 관리](#️-유지-관리)
  - [💬 조회 및 피드백](#-조회-및-피드백)
...
이번 릴리스는 **로컬 모델 호환성**과 **데이터 품질**에 초점을 맞춥니다.

**핵심 개선 사항：**

- **LM Studio 제공자 지원（신규）。** 독립 드롭다운 옵션. API 키는 선택 사항. 기본 URL `http://localhost:1234/v1`。

- **컨텍스트 창 설정.** 로컬 모델 사용자가 LLM 출력 토큰 제한 가능. 4K~1M 드롭다운. 로컬/사용자 정의 제공자만 표시.

- **YAML sources 필드 정규화（Issue #81）。** 6가지 오염 패턴 자동 정규화. Lint의 LLM 의존 단계 전에 실행.

- **시작 시 빠른 수정.** 기존 "시작 시 건강 검사"가 low-level 형식 문제를 자동 수정. 10초 상세 알림. 기본 활성화.

- **별칭 언어 수정.** 하드코딩된 중영 번역 규칙 제거.

- **LM Studio HTTP 400 수정（Issue #75）。** shadow 상수 제거.

- **설정 UI 개선.** 새 "LLM-Wiki 상태" 섹션. 섹션 제목 간소화.
