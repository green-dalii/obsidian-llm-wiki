...
v1.25.0 이후의 핫픽스 릴리즈: LLM 출력 및 lint의 8건의 사일런트 손실 수정, 3건의 대형 파일 분리, 1건의 빌드 검증 근본 원인. v1.25.0 사용자에게 업그레이드를 강력히 권장합니다.

- **🔕 Related 페이지 경로에서 사일런트 데이터 손실 제거.** 이전에는 LLM이 Related 페이지 본문을 재작성할 때 Mentions 섹션을 다시 출력하지 않으면, canonicalizer / related-link corrector / preserveExistingSections 파이프라인이 (처리된 본문이 아닌) LLM 원본 응답 위에서 실행되었습니다. 그래서 재수집 시 누적된 Mentions 내용이 조용히 파괴될 수 있었습니다. Related 경로가 이제 merge 경로와 정렬됩니다: canonicalize → correct → preserveExistingSections. **업그레이드 강력 권장** — 소스별 Mentions 누적 + 재수집에 의존하는 모든 노트가 혜택을 받습니다.
- **🛡️ Schema 섹션이 재작성으로 손실되지 않음.** LLM이 우리 스키마와 어긋나는 지점은 작성하도록 요청된 부분뿐입니다. 페이지에 이미 존재하던 canonical 섹션 블록은 LLM의 재작성이 누락해도 verbatim으로 복원됩니다. merge + related 경로가 공유하는 단일 `preserveExistingSections` 헬퍼로 통합되었습니다.
- **🔗 레거시 Mentions 페이지가 자가 치유 가능.** pre-#244 그룹화 mentions가 파싱 단계에서 인식되어, 다음 수집 시 레거시 페이지가 구조화된 형태로 복원됩니다 — LLM이 중복된 Mentions 블록을 더는 환각해내지 않습니다(프로그래매틱 주입이 유일한 출처).
- **🔌 LM Studio를 API 키 없이 수집 가능.** 로컬 전용 LM Studio(`http://localhost:1234/v1`)는 이제 자리표시 키 없이 수집 가능; 비-LM-Studio 공급자는 여전히 명시적 API 키가 필요합니다(변경 없음).
- **🐢 main.js 비대화 억제, lint 가속화.** 대형 파일 분리: `wiki-engine.ts` 1799 → 더 작게(Phase C-PR1, 4 내부 모듈), `settings.ts` 1439 → 더 작게(Phase C-PR2, 8 섹션 렌더러), `main.ts` 1304 → 300 LOC(Phase C-PR3, 6 main-commands 모듈). `DiskCache<T>`를 추출하고 제한된 증가(총 100 MB / 1000개 / 단일 10 MB 상한 + LRU-by-mtime 축출)를 보장합니다.
- **🛠 빌드 검증 근본 원인.** v1.25.0의 npm 레지스트리 전환의 진짜 원인은 로컬 `pnpm-lock.yaml`과 CI `package-lock.json` 간의 표류였습니다; 두 lockfile이 모두 단일 `node_modules` 스냅샷에서 재생성되어 로컬 빌드와 Obsidian CI 빌드가 동일해집니다.
- **⚠ 오류 발생 시 진행 알림이 사라짐.** 단일 파일 수집(`ingestActiveFile`, `selectSourceToIngest`)이 이제 예외 발생 시 영구적인 `Ingesting: <basename>` Notice를 숨겨, 실패한 수집이 다음 성공할 때까지 상태 표시줄에 남아있는 일이 없어집니다.
- **🧪 2274개 테스트 통과.** v1.25.0의 2182개에서 증가. 신규 모듈: `DiskCache<T>`, `lint-fix-all-completion`, `page-batch-runner`, `graph-cache`, `index-generator`, `log-writer`, `section-header-canonicalizer`.

**점검할 설정:** 없음 — 본 릴리즈는 버그 수정 + 리팩토링뿐; 새 설정, 새 명령, 새 로케일 없음.
