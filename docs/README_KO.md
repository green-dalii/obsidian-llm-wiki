...
5개의 병합된 PR — 버그 수정, 리팩터링, UX 개선. 1431 테스트 통과. 새로운 사용자 노출 설정 없음. v1.23.0+ 모든 사용자에게 업그레이드 권장.

- **🛠️ sources 프롬프트 후보 격리 (#234).** LLM 후보 목록이 기본적으로 `wiki/sources/` 페이지를 제외하여, 약한 로컬 모델이 잘못된 `[[sources/<잘못된slug>|<올바른라벨>]]` 링크를 생성하지 않게 되었고 RAG가 잘못된 페이지로 라우팅되지 않습니다. 프로그램 생성 관련 페이지 매칭(`source-analyzer.ts:421`)은 영향받지 않습니다. Constraints 프롬프트는 이제 후보 목록을 명시적으로 참조합니다.
- **🔄 인제스트 시 PPR 그래프 캐시 무효화.** `wiki/`에 접촉하는 인제스트는 모든 열린 Query 패널의 PPR 그래프 캐시를 무효화합니다 — 동일한 Obsidian 세션에서 인제스트 후 즉시 쿼리할 수 있습니다.
- **🔁 스트리밍 보존 클라이언트 래퍼.** `wrapWithAdvancedSettings`를 컴포지션 패턴(`Object.create(client)` + 명시적 `createMessage` 오버라이드)으로 재작성. 클래스 기반 SDK 클라이언트가 `{ ...client }`로 프로토타입 체인에서 `createMessageStream`을 잃고 비스트리밍으로 폴백하던 v1.23.0 시대 회귀를 제거.
- **🧩 Frontmatter 직렬화기 통합 (DocTpoint, PR #238).** `mergeFrontmatter` / `enforceFrontmatterConstraints` / `mergeDuplicatePages`가 단일 `serializeFrontmatter`에 위임. 동작 변경 없음(YAML 동등), 하지만 곧 출시될 `supersedes:` 플래그(v1.24.0)는 한 곳에만 추가하면 됩니다.
- **🖱️ Query 턴 인디케이터 + 검색 라벨 UX 개선 (#219, #221).** 각 대화 턴의 우측 도트, 호버 시 원래 질문 프리뷰, IntersectionObserver 기반 활성 하이라이트, 클릭 시 스크롤. `🔍 N page(s) · …` 검색 라벨이 클릭 가능해졌습니다 — 클릭하면 검색된 페이지 목록을 인라인으로 확장(Notice 팝업 없음). 생성 완료 시 답변 끝이 아니라 사용자 질문으로 스크롤.
- **📋 시맨틱 기반 진행 알림 (#219).** 수동 작업은 Notice + 상태 표시줄, 백그라운드 작업(watch 모드 자동 인제스트, 주기적 lint, 시작 QuickFixes)은 상태 표시줄만 표시. 새로운 사용자 노출 설정 없음 — 채널 선택은 작업 유형에서 파생. Lint 완료 알림은 이제 `NOTICE_NORMAL` / `NOTICE_ERROR` 시간 초과를 준수하며 수동으로 닫을 때까지 남아있지 않습니다.
- **📝 정규화된 섹션 헤더 (DocTpoint, PR #241).** `wikiLanguage: de`에서 로컬 모델이 섹션 헤더를 잘못 생성하는 경우(예: 정규 라벨 `Erwähnungen in der Quelle`을 `Erwägungen…` / `Erwurnungen…`로 작성)가 경계 Levenshtein 거리로 쓰기 시 정규 라벨로 다시 스냅됩니다 — Tier-B 검색에서 조용히 손실되는 것을 제거.
- **📜 라이선스 업그레이드.** MIT → Apache License 2.0 + DCO. NOTICE 파일에 모든 인간 기여자 나열. 기존 기여는 소급 적용되지 않음; 이후 commit에는 `Signed-off-by:` 포함 필요.
