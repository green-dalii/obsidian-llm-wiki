...
- [⚡ v1.15.0 업데이트 하이라이트](#-v1150-업데이트-하이라이트)
- [✨ 주요 기능](#-주요-기능)
  - [📊 지식 품질](#-지식-품질)
  - [🛠️ 유지 관리](#️-유지-관리)
  - [💬 조회 및 피드백](#-조회-및-피드백)
...
이번 릴리스는 **Wiki 초기화 UX 및 아키텍처 최적화**를 중심으로 합니다. 첫 사용 시 원활한 설정과 지속적인 테스트 인프라 확장에 초점을 맞추었습니다.

**핵심 개선 사항:**

- **Wiki 자동 초기화 (Issue #80).** 첫 LLM 연결 테스트 성공 시 플러그인이 Wiki 폴 더 구조(entities, concepts, sources, schema)를 자동으로 생성합니다. 설정 패널의 상태 표시기(✅/⚠️)로 Wiki 상태를 실시간 확인. 새 Vault에서 "기본 Schema 재생성" 버튼이 응답하지 않던 문제가 해결됩니다.

- **SSE 파서 추출.** 스트리밍 응답 파싱 로직(Anthropic + OpenAI 형식)을 `src/core/sse-parser.ts` 공유 순수 함수로 추출. 두 형식, CRLF 정규화, 잘못된 JSON 허용, `[DONE]` 종결자를 커버하는 11개 테스트.

- **잘림 재시도 추출.** 토큰 잘림 재시도 정책(`stop_reason=max_tokens` 또는 `finish_reason=length` 감지, max_tokens 2배로 재시도)을 `src/core/truncation-retry.ts`로 통합. LLM 클라이언트 간 중복 코드 3개 제거. 상한 동작, 오류 전파, 경고 로깅을 커버하는 7개 테스트.

- **테스트 인프라 성장.** +37 테스트(총 446개, 21개 테스트 파일). AnthropicClient 잘림 재시도 테스트(9개, prefill 중괄호 복원, MAX_TOKENS_BATCH 상한, cacheBreakpoint 통과 포함). Wiki 초기화 테스트(10개, 순수 모의, Obsidian 런타임 불필요).

- **개발 품질 폐쇄 루프.** TDD + 계획 루프를 CLAUDE.md에 공식 문서화하고 실제 위반 사례(2026-06-02) 추가. 모든 신규 코드 변경은 9단계 폐쇄를 따릅니다: 심층 사고 → 계획 → 테스트 작성 → RED 확인 → 구현 → GREEN 확인 → 리팩터링 → 4-Gate 검증 → 3-No 검토.

**이전 버전에서 업그레이드?** 그냥 설치하고 사용하면 됩니다——파괴적인 변경 사항이 없습니다. 기존 Wiki 페이지, 설정 및 워크플로가 모두 보존. 재구성 불필요.

**모든 사용자에게 이 버전으로 업그레이드할 것을 강력히 권장합니다.**
