...
- **国际化** — 中英文界面切换（默认英文）
- **Schema 层** — 结构化配置（`wiki/schema/config.md`）注入所有 LLM 提示词，确保 Wiki 输出一致、高质量
...
**用户反馈闭环（审阅页面）：** 在 Wiki 页面的 frontmatter 中添加 `reviewed: true`，标记该页已手动审阅。重新摄入时，插件会完整保留用户编辑的所有内容，仅追加不重复的新信息。保护你的手动编辑不被 LLM 重新生成所覆盖。
