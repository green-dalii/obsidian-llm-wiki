...
- **摄入加速** — 可配置的页面生成并行度（1-5），批次间延迟防止 API 限流。50+ 实体长文档提速 3 倍
- **原文引用保留** — 来源提及部分保留原文语言，可在括号内添加翻译
- **实体/概念关联增强** — 独立的"相关实体"和"相关概念"章节，支持双向关系追踪
- **多 LLM Provider 支持** — Anthropic (Claude)、Anthropic 兼容（Coding Plan）、Google Gemini、OpenAI、DeepSeek、Kimi、GLM、OpenRouter、Ollama，以及自定义 OpenAI 兼容接口
- **Wiki 输出语言** — 8 种语言可选（英/中/日/韩/德/法/西/葡），独立于界面语言，支持自定义输入
- **国际化** — 中英文界面切换（默认英文）
- **Schema 层** — 结构化配置（`wiki/schema/config.md`）包含明确的章节模板、融合策略和内容规范，注入所有 LLM 提示词
- **智能摄入** — 自动从源文档提取实体和概念，生成带双向 `[[wiki-links]]` 的结构化 Wiki 页面
- **内容截断保护** — 所有页面生成 max_tokens 提升至 8000，`stop_reason`/`finish_reason` 检测并自动以 2x token 重试，覆盖全部 LLM provider
- **迭代批量提取** — 分批提取实体/概念并自适应调整批次大小，消除长文档的 max_tokens 瓶颈
- **智能知识融合** — 更新已有页面时，LLM 对比新旧内容，检测重复和矛盾，进行结构化增量合并
- **对话式查询** — ChatGPT 风格对话框，流式 Markdown 输出，支持多轮追问和保存到 Wiki
- **查询到 Wiki 反馈** — 查询关闭时进行 3 阶段价值评估，保存前经 LLM 语义去重
- **Lint AI 自动修复** — LintReportModal 中逐项修复死链、空洞页面、孤立页面
- **矛盾状态机** — `检测 → 审核通过 → 已解决`（AI 修复）或 `检测 → 待修复`（手动）
- **自动维护** — 文件监听自动摄入、定时 Lint（每小时/每天/每周）、启动健康检查
- **知识图谱** — 在 Obsidian Graph View 中可视化实体/概念关系
- **自动索引** — `index.md` 和 `log.md` 自动维护，支持按标签分层组织
